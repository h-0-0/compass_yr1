...
&= \gamma^{T} (K_{m}^{0})^{-1} K(X_{1:m}, x)
\end{aligned}
$$
where $K(A,B) = [k(a_{i}, b_{j})]_{i,j}$, X is our data matrix where $X_{1:m}$ means the data matrix including only the first m datapoints and we let $\gamma^{T} = \beta_{\lambda}^{T} K(X_{1:m}, X)^{T}$. Let's now rewrite the penalty:
$$
\begin{aligned}
...
&= - \lambda \gamma^{T} (K_{m}^{0})^{-1} \gamma
\end{aligned}
$$
This leaves us with the following optimization problem,
$$
\arg \max_{\alpha \in \mathbb{R}, \phi \in (0, \inf), \gamma \in \mathbb{R}^{m}} \frac{1}{2n} \sum_{i=1}^{n} \log f(y_{i} ; g^{-1}(\alpha + \gamma^{T} (K_{m}^{0})^{-1} K(X_{1:m}, x_{i}^{0}) ), \phi ) - \lambda \gamma^{T} (K_{m}^{0})^{-1} \gamma
$$

### Question 5
Let's first find the spectral decomposition of $(K_{m}^{0})^{-1}$,
$$
(K_{m}^{0})^{-1} = S \Lambda S^{-1}
$$
Then we can write,
$$
\begin{aligned}
\gamma^{T} (K_{m}^{0})^{-1} \gamma &{}= \gamma^{T} S^{-T} \Lambda S^{-1} \gamma 
&= || \Lambda^{\frac{1}{2}} S^{-1} \gamma ||_{2}^{2}
\end{aligned}
$$
Glmnet estimates parameters that minimize the following (if using the ridge penalty):
$$
- \frac{1}{n} \sum_{i=1}^{n} \log f(y_{i}; g^{-1}(\alpha + \gamma^{T} X_{i} ), \phi) + \frac{\lambda}{2} || \gamma_{glm} ||_{2}^{2} 
$$
